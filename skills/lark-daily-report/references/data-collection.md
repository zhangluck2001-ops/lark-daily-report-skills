...
所有素材都通过 `lark-cli` 抓取；没有外部数据源。`scripts/collect-sources.mjs` 只是把多条飞书 CLI 调用编排成一次采集，并负责分页、并发、脱敏和结构压缩。
...
- 临时文件权限为 `0600`，仅保存脱敏后的结构化素材。

## 低 token 工作集

先看索引。索引只输出日程、会议、文档、数据缺口和少量候选会话：

```bash
node scripts/inspect-sources.mjs --input "$SOURCES_FILE" --index --limit-chats 8
```

候选排序只用于节省上下文，不是最终清洗判断。AI 仍要结合日程、会议、文档和会话上下文判断工作相关性。
...
- 不要一次性打印完整临时文件。
- 不要为了“保险”展开所有会话；优先展开候选分高、与日程/会议/文档互相印证、或上下文不确定的会话。
...
| 当天全部群聊与私聊 | `lark-cli im +chat-list --as user --types group,p2p` |
